...
class Agent2Controller extends \common\controllers\aController
{
    use Agent2Helper;
                    ...
                    if( $user->save( $attr ) && $parentUser->save( $attr )
                        ...
                        && $this->updateTransactionHistory($parentUser->id,$r_data['chips'],$pType,$description2,$remark)
                            ...
                            $response = [
                                'status' => 1 ,
                                "success" => [
                                    "message" => "Master balance updated successfully!"
                                ]
                            ];
                    }else{
                        $response[ "error" ] = [
...
trait Agent2Helper{
    protected function doIndex(){
        ...
        $query = User::find()
        ->select( [ 'u.id' , 'u.parent_id','name' , 'username' , 'profit_loss' ,'profit_loss_balance','commission' ,'balance' , 'remark', 'u.created_at' , 'u.updated_at' , 'u.status','u.isbet' ] )
            ->from( User::tableName() . ' u' )
            ->innerJoin( 'auth_assignment auth' , "u.id=auth.user_id" )
            ->andOnCondition( "auth.item_name like 'agent2'" )
            ->andWhere( $whare )
            ->asArray();
    ...
    protected function profitLossAmountBYId($uid)
    {
    ...
    protected function superMasterConntBYId($uid)
    {
        $count = User::find()
        ->select( ['id'] )
    ...
    protected function masterConntBYId($uid)
    {
        $count = User::find()
        ->select( ['id'] )
    ...
    protected function clientConntBYId($uid)
    {
        $count = User::find()
        ->select( ['id'] )
    ...
    protected function doReference(){
...
//        }
        ...
        $query = User::find()
        ->select( [ 'u.id' , 'name' , 'username' , 'profit_loss' ,'commission' ,'balance' ,'remark','profit_loss_balance', 'u.created_at' , 'u.updated_at' , 'u.status' ] )
        ->from( User::tableName() . ' u' )
        ->innerJoin( 'auth_assignment auth' , "u.id=auth.user_id" )
        ->andOnCondition( "auth.item_name like 'agent2'" )
        ->andWhere( $whare )
    ...
    protected function doCreate(){
        ...
        if( $user != null && ( ( $user->role != 1
            && ( ($user->profit_loss+1) > $r_data[ 'profit_loss' ] )
            ...
            if( isset($role['agent2']) && $role['agent2'] != null ){
                $error = $this->checkMasterCreateLimit($user);
                if( $error == 'Error1' ){
                    return [ "status" => 0 , "error" => [ "code" => 400 , "message" => "You can not create master user!!" ] ];
                }
                if( $error == 'Error2' ){
                    return [ "status" => 0 , "error" => [ "code" => 400 , "message" => "You have added max limit(3) of master user!" ] ];
                }
            }
                    ...
                    if( $user->save() ){
                        ...
                        if( $this->updateTransactionHistory($model->id,$r_data['balance'],$uType,$description1,$r_data[ 'remark' ])
                            ...
                            $response = [
                                'status' => 1 ,
                                "success" => [
                                    "message" => "New master saved successfully!"
                                ]
                            ];
                        }else{
    ...
    protected function checkMasterCreateLimit($user){
        ...
        if( $user != null ){
            ...
            ->where(['parent_id'=> $user->id , 'role' => '3' , 'status'=>[1,2]])
            ->asArray()->all();
                ...
                if( $user != null ){
                    ...
                    if( $user->save( [ 'status','is_login' ] ) ){
                        ...
                        $response = [
                            'status' => 1 ,
                            "success" => [
                                "message" => "Master $sts successfully!"
                            ]
                        ];
                    }else{
                        $response[ "error" ] = [
    ...
    protected function doView(){
        ...
        if( $id != null ){
            $model = User::find()->select( [ 'id' , 'username' , 'name' , 'commission' , 'profit_loss' , 'max_stack' , 'min_stack' , 'max_profit_limit' ] )->where( [ 'id' => $id ] )->asArray()->one();
            if( $model != null ){
                $response = [ "status" => 1 , "data" => $model ];
            }
        }
