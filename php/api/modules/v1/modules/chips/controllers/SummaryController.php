...
//                        }
                    ...
                    if ($user->save(['profit_loss_balance'])) {
                        if ($this->clearChipsTransaction($user, $amount, $r_data['typ'], $r_data['remark']) == true) {
                            ...
                            //if( $this->updateChipsTransaction( $user , $r_data ) == true ){
    ...
    public function updateChipsTransaction($user, $data)
    {
        ...
        if (isset($data['typ']) && ($data['typ'] == 'loss')) {
            ...
            $resultArr[] = [$user->id, $user->parent_id, 0, 0, 0, 'DEBIT', $amount, ($user->balance + $user->profit_loss_balance - $amount), 'Withdraw By ' . $pUser->username, $data['remark'], 1, time(), time()];
            $resultArr[] = [$user->id, $user->parent_id, 0, 0, 0, 'CREDIT', $chips, ($user->balance + $user->profit_loss_balance + $amount), 'Deposit By ' . $pUser->username, $data['remark'], 1, time(), time()];
    ...
    public function clearChipsTransaction($user, $amount, $typ, $remark)
    {
        ...
        if ($typ == 'loss') {
            ...
            if ($user->role == 4) {
                $current_balance = $user->balance + $user->profit_loss_balance;
            } else {
