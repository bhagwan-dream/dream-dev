...
class TennisController extends \common\controllers\aController
{
            ...
            if( isset( $r_data['filter'] ) ){
                ...
                if( $filter['status'] != null || $filter['title'] != null){
                    $inplay = EventsPlayList::find()
                        ->where(['game_over' => $gameOver , 'status'=>[1,2] , 'sport_id' => 2 , 'play_type' => $filter['status'] ])
                        ->andWhere(['LIKE','event_name',$filter['title']])
                        ->orderBy( [ 'event_time' => SORT_DESC ] )->asArray()->all();
                }else{
                    $inplay = EventsPlayList::find()
                        ->where(['game_over' => $gameOver , 'status'=>[1,2] , 'sport_id' => 2 ])
...
//        ->orderBy( [ 'event_time' => SORT_DESC ] )->asArray()->all();
    ...
    public function actionTrash()
    {
        $inplay = [];
        $inplay = EventsPlayList::find()
        ->where(['game_over' => ['YES','NO'] , 'status' => 0, 'sport_id' => 2 ])
        ->orderBy( [ 'event_time' => SORT_DESC ] )->asArray()->all();
