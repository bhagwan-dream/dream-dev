...
class AppMyFavoriteController extends \common\controllers\aController  // \yii\rest\Controller
{
    ...
    public function actionEventList()
    {
        ...
        if( null !== \Yii::$app->user->id ){
                ...
                foreach ( $betListEvent as $event ){
                    ...
                    if( $eventData != null ){
                        ...
                        if( $sportId == '1' ){
                            ...
                            if( $matchoddData != null || $matchodd2Data != null || $fancy2Data != []
                                || $fancyData != [] || $lotteryData != [] ) {
            ...
            $response = [ "status" => 1 , "data" => [ "items" => $eventArr , 'marketIdsArr' => $marketIdsArr ] ];
        }
    ...
    public function getDataMatchOdd($uid,$marketId,$eventId)
    {
        ...
        ->where(['user_id'=>$uid,'market_type' =>'match_odd','event_id'=>$eventId])
        ->one();
    ...
    public function getDataManualMatchOdd($uid,$eventId,$title)
    {
        $betMatchOdd = (new \yii\db\Query())
        ->select(['market_id'])->from('favorite_market')
        ->where(['user_id'=>$uid,'market_type' =>'match_odd2','event_id'=>$eventId])
        ->one();
    ...
    public function getDataFancy($uid,$eventId,$title)
    {
        ...
        ->select(['market_id'])->from('favorite_market')
        ->where(['user_id'=>$uid,'market_type' =>'fancy2','event_id'=>$eventId])
    ...
    public function getDataManualSessionFancy($uid,$eventId,$title)
    {
        $betListFancy = (new \yii\db\Query())
        ->select(['market_id'])->from('favorite_market')
        ->where(['user_id'=>$uid,'market_type' =>'fancy','event_id'=>$eventId])
        ->groupBy(['market_id'])->all();
    ...
    public function getDataLottery($uid,$eventId,$title)
    {
        ...
        ->select(['market_id'])->from('favorite_market')
        ->where(['user_id'=>$uid,'market_type' =>'lottery','event_id'=>$eventId])
        ->groupBy(['market_id'])->all();
    ...
    //Event: My Favorite Manual Session MatchOdd
    public function actionManualSessionMatchOdd()
    {
        if( null !== \Yii::$app->user->id && null !== \Yii::$app->request->get( 'id' ) ){
            ...
            if( $betMatchOdd != null ){
                $marketId = $betMatchOdd['market_id'];
                //$isFavorite = $this->isFavorite($eventId,$marketId,'match_odd2');
                $market = (new \yii\db\Query())
                ...
                if( $marketList != null ){
                    ...
                    foreach ( $marketList as $market ){
                        ...
                        $items[] = [
                            ...
                            'slug' => 'cricket',
                            'is_book' => $this->isBookOn($market['market_id'],'fancy2'),
                            'is_favorite' => '1',//$this->isFavorite($market['event_id'],$market['market_id'],'fancy2'),
                            'data' => $dataVal,
                        ];
                    }
    ...
    //Event: My Favorite Manual Session Fancy
                ...
                if($marketList != null){
                    ...
                    foreach($marketList as $data){
                        ...
                        $items[] = [
                            ...
                            'slug' => 'cricket',
                            'is_book' => $this->isBookOn($data['market_id'],'fancy'),
                            'is_favorite' => '1',//$this->isFavorite($data['event_id'],$data['market_id'],'fancy'),
    ...
    //Event: My Favorite Manual Session Lottery
