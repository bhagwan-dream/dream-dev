...
class TennisController extends \common\controllers\aController  // \yii\rest\Controller
{
    ...
    //Tennis - Inplay Today Tomorrow
    public function actionInplayTodayTomorrow()
    {
        $dataArr = $tennisInplay = $tennisToday = $tennisTomorrow = [];
    ...
    public function getLossUnMatchOdds($marketId)
    {
    ...
    // Tennis: getProfitLossUnMatchOdds
    public function getProfitLossUnMatchOdds($marketId,$eventId,$selId,$sessionType)
    {
        $userId = \Yii::$app->user->id;
        $total = 0;
        // IF RUNNER WIN
        if( null != $userId && $marketId != null && $eventId != null && $selId != null){
            ...
            $where = [ 'market_id' => $marketId , 'user_id' => $userId, 'status' => 1 , 'bet_status' => 'Pending', 'sec_id' => $selId , 'event_id' => $eventId , 'bet_type' => 'back' ,'session_type' => $sessionType ];
            ...
            $where = [ 'market_id' => $marketId ,'session_type' => $sessionType, 'user_id' => $userId, 'status' => 1 , 'bet_status' => 'Pending', 'event_id' => $eventId , 'bet_type' => 'lay' ];
                ...
                if( $r_data['session_type'] == 'match_odd'){
                    ...
                    if ($model->load($data)) {
                        ...
                        if( $model->event_id != null ){
                            $play = EventsPlayList::findOne(['sport_id' => 2,'event_id' => $model->event_id , 'market_id' => $model->market_id ]);
                        ...
                        /*
                         $currentBalance = $this->currentBalance($model->match_unmatch,$uid,$eventId,$marketId,$secId,$sessionType,$type);
                        ...
                        ->where(['sport_id' => 2,'event_id' => $model->event_id , 'market_id' => $model->market_id , 'status' => 1 ])->asArray()->one();
    ...
    public function getUserBalanceUpdate($data,$uid)
    {
        ...
        if( $event != null ){
            ...
            if( $runnersData != null ){
                ...
                foreach ( $runnersData as $runners ){
                    $profitLoss = $this->getProfitLossMatchOddsNew($marketId,$eventId,$runners->selection_id,'match_odd',$data);
                    //echo $profitLoss;die;
                    if( $profitLoss < 0 ){
                        $balExpose[] = $profitLoss;
                    }else{
            ...
            if( ( $data['bet_type'] == 'lay' && ( $profitLossNew[$data['sec_id']] > 0 ) ) || ( $data['bet_type'] == 'back' && ( $profitLossNew[$data['sec_id']] < 0 ) ) ){
                $available = $plus+((-1)*$expose)+$available;
                
                $lossUnMatchOdd = $this->getLossUnMatchOdds($marketId);
    ...
    public function getUserBalanceUpdateOld($data,$uid)
    {
        //Match Odd PL
        $marketId = $data['market_id'];
        $eventId  = $data['event_id'];
    ...
    public function actionUnMatchToMatchForMatchOdds()
    {
        if( null != \Yii::$app->request->get( 'id' ) ){
            ...
            $url = 'https://api.betsapi.com/v1/betfair/ex/event?token=15815-peDeUY8w5a9rPq&event_id='.$eventId;
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            $responseData = curl_exec($ch);
            ...
            foreach ( $responseData->results as $results ){
                
                if( !empty( $results->markets ) && !empty( $results->markets ) ){
                    
                    $markets = $results->markets[0];
                    ...
                    if( isset( $markets ) && !empty( $markets ) ){
                        ...
                        foreach ( $markets->runners as $runners ){
                            ...
                            if( isset( $runners->exchange->availableToLay ) && !empty( $runners->exchange->availableToLay ) ){
                                foreach ( $runners->exchange->availableToLay as $layArr ){
                                    $this->updateUnmatchedData($eventId, $marketId, 'lay', number_format($layArr->price , 2), $selectionId);
                                }
                            }
    ...
    public function currentBalance($matchType,$uid,$eventId,$marketId,$secId,$sessionType,$type)
    {
        $user_balance = 0;
        $user = User::find()->select(['balance','expose_balance'])->where(['id' => $uid ])->one();
        ...
        if( $user != null ){
            ...
            /*if( $expose_balance > $user_balance ){
                $expose_balance = $user->balance;
            }*/
                ...
                if( $matchType != 0 ){
                    if( $profitLoss != '' ){
                        if( $profitLoss > 0 && $type == 'lay' ){
                            $user_balance = $user_balance+$expose_balance+$profitLoss;
                        }
                    ...
                    $allProfitLossUnmatch = $this->getAllProfitLossUnMatchOdds();
                    
                    $expose_balance = $expose_balance-$allProfitLossUnmatch;
                    
                    //$profitLossUnmatch = $this->getProfitLossUnMatchOdds($marketId,$eventId,$secId,$sessionType);
                    
                    if( $profitLoss != '' ){
                        
                        if( $profitLoss > 0 && $type == 'lay' ){
                            $user_balance = $user_balance+$profitLoss+$expose_balance-$allProfitLossUnmatch;
                        }
                        if( $profitLoss < 0 && $type == 'back' ){
                            $profitLoss = (-1)*$profitLoss;
                            $user_balance = $user_balance+$profitLoss+$expose_balance-$allProfitLossUnmatch;
    ...
    // Function to get the client current Balance
    public function currentBalanceNew($matchType,$uid,$eventId,$marketId,$secId,$sessionType,$type)
    {
        $user_balance = 0;
        $user = User::find()->select(['balance','expose_balance'])->where(['id' => $uid ])->one();
