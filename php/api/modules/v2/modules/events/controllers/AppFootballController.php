...
class AppFootballController extends \common\controllers\aController  // \yii\rest\Controller
{
    ...
    public function actionIndex()
    {
        ...
        //$eventData = EventsPlayList::findAll(['sport_id'=>1,'play_type'=>['IN_PLAY','UPCOMING'],'game_over'=>'NO','status'=>1]);
        $eventData = (new \yii\db\Query())
            ...
            ->where(['sport_id'=>1,'game_over'=>'NO','play_type' => ['IN_PLAY','UPCOMING','CLOSED'],'status'=>1])
            ...
            if( $r_data['session_type'] == 'match_odd' ){
                ...
                if ($model->load($data)) {
                    ...
                    if( $model->event_id != null ){
                        $play = EventsPlayList::findOne(['sport_id' => 1,'event_id' => $model->event_id , 'market_id' => $model->market_id , 'status' => 1 ]);
                    ...
                    ->where(['sport_id' => 1,'market_id' => $model->market_id,'event_id' => $model->event_id , 'status' => 1 ])->asArray()->one();
