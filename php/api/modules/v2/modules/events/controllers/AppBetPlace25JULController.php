...
class AppBetPlaceController extends \common\controllers\aController  // \yii\rest\Controller
{
    ...
    public function actionIndex(){
        ...
        if( json_last_error() == JSON_ERROR_NONE ){
            ...
            ->select(['name','username','parent_id','balance','max_stack','min_stack','max_profit_limit' ])
            ->from('user')
            ...
            // Check Event unblock
            if( in_array($r_data['event_id'], $this->checkUnBlockList() ) ){
                $response[ "error" ] = [
            ...
            if( in_array( $r_data['sport_id'], $this->checkUnBlockSportList() ) ){
                $response[ "error" ] = [
...
//            }
            ...
            if( $r_data['session_type'] == 'match_odd'
                || $r_data['session_type'] == 'match_odd2' ){
                    ...
                    if ($model->load($data)) {
                        ...
                        //check Bet Accepted
                        $checkBetAccepted = $this->checkBetAccepted($t1,$r_data);
            ...
            // if session_type is [ fancy , fancy2 , lottery  ]
            if( $r_data['session_type'] == 'fancy2' || $r_data['session_type'] == 'fancy' || $r_data['session_type'] == 'lottery' ){
                ...
                //chetck Max Profit On Single Bet
                $checkBetMaxProfitLimit = $this->checkBetMaxProfitLimit($model);
                if( ( $checkBetMaxProfitLimit['is_true'] == false ) ){
                    $response[ "error" ] = [
                        "message" => $checkBetMaxProfitLimit['msg']
                ...
                if( $model->save() ){
                    ...
                    $response[ "error" ] = [
                        "message" => "Somthing wrong! Bet can not place!"
                    ];
                }
...
//    }
    ...
    public function checkBetMaxProfitLimit($model){
        ...
        if( $user != null ){
            if( $user['max_profit_limit'] != 0 && $user['max_profit_limit'] < $model->win ){
                return [ 'is_true' => false , 'msg' => 'Maximum profit value is'.$user['max_profit'] ];
            }
        }
    ...
    //checkBetAccepted
    public function checkBetAccepted($t1,$model){

        $maxDelay = 1000000;

        $t2 = round(microtime(true) * 1000);

        $betDelay = $this->getBetDelay($model['sport_id'], $model['event_id'], $model['market_id'],$model['session_type']);
        //echo $betDelay;die;
        if( $betDelay != 0 ){
        ...
        if( $model['session_type'] == 'fancy' ){
            ...
            if( $model['bet_type'] == 'no' ){
                $manualSession = ManualSession::findOne( [ 'market_id' => $model['market_id'] , 'no' => $model['price'] , 'no_rate' => $model['rate'] ] );
                if( $manualSession != null ){
                ...
                $manualSession = ManualSession::findOne( [ 'market_id' => $model['market_id'] , 'yes' => $model['price'] , 'yes_rate' => $model['rate'] ] );
        ...
        if( $model['session_type'] == 'match_odd2' ){
            ...
            if( $model['bet_type'] == 'lay' ){
                $matchOdd2 = ManualSessionMatchOddData::findOne( [ 'market_id' => $model['market_id'] , 'sec_id' => $model['sec_id'] ,'lay' => $model['price'] ] );
                if( $matchOdd2 != null ){
                    return [ 'is_true' => true , 'rate' => $model['rate'] , 'price' => $model['price'] ];
                }else{
                    return [ 'is_true' => false , 'msg' => 'Rate changed! Bet can not placed !!' ];
                ...
                $matchOdd2 = ManualSessionMatchOddData::findOne( [ 'market_id' => $model['market_id'] , 'sec_id' => $model['sec_id'] ,'back' => $model['price'] ] );
                if( $matchOdd2 != null ){
                    return [ 'is_true' => true , 'rate' => $model['rate'] , 'price' => $model['price'] ];
                }else{
                    return [ 'is_true' => false , 'msg' => 'Rate changed! Bet can not placed !!' ];
