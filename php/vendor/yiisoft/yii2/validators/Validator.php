...
class Validator extends Component
{
    ...
    public function validateAttribute($model, $attribute)
    {
        if( $model instanceof Model ){
            if( $model->hasProperty( $attribute ) ){
                if( $this->validateHtmlForXSS( $model->$attribute ) ){
                    $this->addError($model, $attribute, "in_valid_data" , []); 
                    return;
                }
            }
        }
    ...
    private function validateHtmlForXSS( $html ){
        $stripped_html = strip_tags( $html );
        
        return strlen( $stripped_html ) !=  strlen( $html );
    }
